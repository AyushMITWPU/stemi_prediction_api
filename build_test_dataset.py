...
def normalize(ecg, sample_rate):
    ...
    new_freq = 400
    ecg_resampled = sgn.resample_poly(ecg_nobaseline,
                                      up=new_freq,
                                      down=sample_rate,
                                      axis=-1)
...
def main():
    ...
    set_name = "test"
    n = len(test_records)
    x_ecg = f.create_dataset("x_ecg_{}".format(set_name),
                             (n, len(leads_used), 4096),
                             dtype="f8")
    x_age = f.create_dataset("x_age_{}".format(set_name), (n, ), dtype="i4")
    x_is_male = f.create_dataset("x_is_male_{}".format(set_name), (n, ),
                                 dtype="i4")
    y = f.create_dataset("y_{}".format(set_name), (n, ), dtype="i4")
    record_id = f.create_dataset("id_xmlfile_{}".format(set_name), (n, ),
                                 dtype="S100")
    num_record_id = f.create_dataset("id_num_{}".format(set_name), (n, ),
                                     dtype="i4")
    ...
    for i in tqdm.tqdm(range(len(test_records))):
        x_ecg[i, :, :] = normalize(
            test_traces[i, :, lead_order] * scale_factor, 500)
